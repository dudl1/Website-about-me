...
const my_stack_json = [
    ...
    },
    {
        ...
        title: "Dart",
        more: "Использую Flutter для создания в первую очередь Android приложение и Web приложений."
    },
    {
    ...
    },
    {
        ...
        title: "Supabase",
        more: "Всегда настроивал базу данных для своих нужд (регистрация пользователей, отправка сообщений и картинок в реальном времени)."
    },
    {
    ...
    },
    {
        ...
        title: "Github",
        more: "Сохраняю проекты. Знаю базовые команды git. Иногда использую Github pages."
    }
];
...
blok_stack_skill.forEach(header => {
    ...
    header.addEventListener('click', () => {
        ...
        if (blok.classList.contains('active')) {
            const confetti_url = blok.attributes[1].value;
    
            const appendCanvas = document.querySelector("canvas");
            if (appendCanvas) {
                blok.appendChild(appendCanvas);
            }

            (async () => {
                blok.confetti = blok.confetti || (await confetti.create(blok, { resize: true }));
